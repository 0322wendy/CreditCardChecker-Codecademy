...
function validateCred(arr) {
    ...
    for (let i = arr.length - 1; i >= 0; i--) {
      if ((arr.length % 2 === 0) && (i % 2 === 0 || i === 0) && (i !== arr.length - 1)) {
        let num = arr[i] * 2;
        if (num > 9) {
          num = num - 9;
          validateArr.unshift(num);
        }
        else {
          validateArr.unshift(num);
        }
      }
      else if ((arr.length % 2 !== 0) && (i !== arr.length - 1) && (i % 2 !== 0)) {
        let num = arr[i] * 2;
        if (num > 9) {
          num = num - 9;
          validateArr.unshift(num);
        ...
        validateArr.unshift(arr[i]);
      }
    }
    //console.log(validateArr);
    const initialValue = 0;
    const sum = validateArr.reduce((previousValue, currentValue) => previousValue + currentValue, initialValue);
    //console.log(sum);
    if (sum % 10 === 0) {
      return true;
    }
